...
try{
    ...
    $sql="SELECT s.*,(SELECT COUNT(*) FROM appointments a WHERE a.matrix_number=s.matrix_number) as total_appointments,(SELECT COUNT(*) FROM queue q WHERE q.matrix_number=s.matrix_number) as total_visits,(SELECT MAX(q2.created_at) FROM queue q2 WHERE q2.matrix_number=s.matrix_number) as last_visit FROM students s";
    if($where) $sql.=" WHERE ".implode(" AND ",$where);$sql.=" ORDER BY s.created_at DESC";
    $stmt=$pdo->prepare($sql);$stmt->execute($params);$students=$stmt->fetchAll(PDO::FETCH_ASSOC);
...
}catch(PDOException $e){$students=[];$total_students=$new_this_month=0;}
$dash=($_SESSION['role']==='Admin')?'admin_dashboard.php':'staff_dashboard.php';
?>
<!DOCTYPE html><html lang="en"><head>
    <meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Students — PKU UTHM</title>
    ...
    <style>
        *,*::before,*::after{margin:0;padding:0;box-sizing:border-box}
        :root{--blue-900:#0c2461;--blue-600:#1d5bb5;--blue-500:#2563eb;--blue-50:#eff6ff;--blue-100:#dbeafe;--green-600:#16a34a;--green-100:#dcfce7;--amber-600:#d97706;--amber-100:#fef3c7;--red-600:#dc2626;--red-100:#fee2e2;--purple-600:#7c3aed;--purple-100:#ede9fe;--slate-900:#0f172a;--slate-700:#334155;--slate-500:#64748b;--slate-300:#cbd5e1;--slate-200:#e2e8f0;--slate-100:#f1f5f9;--slate-50:#f8fafc;--white:#fff;--sidebar-w:232px;--topbar-h:58px;--font:'Plus Jakarta Sans',sans-serif}
        ...
        .stat-icon{width:40px;height:40px;border-radius:9px;background:var(--blue-50);display:flex;align-items:center;justify-content:center;font-size:17px;flex-shrink:0;color:var(--blue-600)}
        .stat-val{font-size:24px;font-weight:800;letter-spacing:-1px;line-height:1;color:var(--blue-600)}
        ...
        .data-table th{padding:9px 14px;text-align:left;font-size:10.5px;font-weight:700;text-transform:uppercase;letter-spacing:.8px;color:var(--slate-400);border-bottom:1px solid var(--slate-100)}
...
<aside class="sidebar">
    ...
    <nav class="sidebar-nav">
        ...
        <a href="<?php echo $dash; ?>" class="nav-link"><span class="ico"><span class="material-symbols-outlined">dashboard</span></span> Dashboard</a>
        ...
        <a href="student_list.php" class="nav-link active"><span class="ico"><span class="material-symbols-outlined">dashboard</span></span> Students</a>
...
<header class="topbar">
    <div><div class="topbar-title">Student List</div><div class="breadcrumb"><a href="<?php echo $dash; ?>">Dashboard</a><span class="sep">›</span><span>Students</span></div></div>
    <div class="topbar-right">
        <div class="topbar-date"><?php echo date('D, d M Y'); ?></div>
        <a href="register_user_form.php" class="btn btn-primary" style="padding:7px 14px;font-size:12.5px;">+ Register Patient</a>
    </div>
</header>
<div class="page"><div class="page-body">
    <div class="page-intro"><h1>Student List</h1><p>All registered students and their visit history</p></div>
    <div class="stats-row">
        <div class="stat-card"><div class="stat-icon"><span class="material-symbols-outlined">school</span></div><div><div class="stat-val"><?php echo $total_students; ?></div><div class="stat-lbl">Total Students</div></div></div>
        <div class="stat-card"><div class="stat-icon"><span class="material-symbols-outlined">star</span></div><div><div class="stat-val"><?php echo $new_this_month; ?></div><div class="stat-lbl">New This Month</div></div></div>
        <div class="stat-card"><div class="stat-icon"><span class="material-symbols-outlined">search</span></div><div><div class="stat-val"><?php echo count($students); ?></div><div class="stat-lbl">Search Results</div></div></div>
    </div>
    <div class="filter-bar">
        <form method="GET"><div class="filter-row">
            ...
            <div class="fg"><label>Faculty</label><select name="faculty"><option value="">All Faculties</option><option value="FKEE" <?php if($faculty==='FKEE') echo 'selected'; ?>>FKEE</option><option value="FKAAB" <?php if($faculty==='FKAAB') echo 'selected'; ?>>FKAAB</option><option value="FKMP" <?php if($faculty==='FKMP') echo 'selected'; ?>>FKMP</option><option value="FSKTM" <?php if($faculty==='FSKTM') echo 'selected'; ?>>FSKTM</option><option value="FPTP" <?php if($faculty==='FPTP') echo 'selected'; ?>>FPTP</option><option value="FPTV" <?php if($faculty==='FPTV') echo 'selected'; ?>>FPTV</option><option value="OTHER">Other</option></select></div>
            <div class="fg"><label>Year</label><select name="year"><option value="">All Years</option><option value="1" <?php if($year==='1') echo 'selected'; ?>>Year 1</option><option value="2" <?php if($year==='2') echo 'selected'; ?>>Year 2</option><option value="3" <?php if($year==='3') echo 'selected'; ?>>Year 3</option><option value="4" <?php if($year==='4') echo 'selected'; ?>>Year 4</option></select></div>
            <div style="display:flex;gap:8px;align-items:flex-end"><button type="submit" class="btn btn-primary" style="padding:9px 16px;">Search</button><a href="student_list.php" class="btn btn-outline" style="padding:9px 14px;">Clear</a></div>
        </div></form>
    </div>
    <div class="table-card">
        <div class="table-head"><h2>Students</h2><span>Showing <?php echo count($students); ?></span></div>
        <?php if(empty($students)): ?><div class="empty-state"><div style="font-size:32px;margin-bottom:10px;opacity:.4"><span class="material-symbols-outlined">inbox</span></div><p>No students found</p></div>
        <?php else: ?><div style="overflow-x:auto"><table class="data-table">
            ...
            <tr>
                ...
                <td style="text-align:center;font-weight:600;color:var(--blue-600)"><?php echo $s['total_visits']; ?></td>
                <td style="color:var(--slate-400);font-size:12px"><?php echo $s['last_visit']?date('d M Y',strtotime($s['last_visit'])):'Never'; ?></td>
                <td><button class="btn btn-sm btn-outline" onclick='showHistory(<?php echo json_encode($s); ?>)'>History</button></td>
            </tr>
            <?php endforeach; ?>
            </tbody>
        </table></div><?php endif; ?>
...
<div class="modal-overlay" id="modal" onclick="if(event.target===this)this.classList.remove('active')">
    <div class="modal">
        <div class="modal-hd"><h3>Patient History</h3><button class="modal-close" onclick="document.getElementById('modal').classList.remove('active')">&times;</button></div>
...
function showHistory(s){
    ...
    document.getElementById('modal-content').innerHTML=`
        <div class="det-row"><span>Matrix</span><span>${s.matrix_number}</span></div>
    ...
    fetch(`get_patient_history.php?matrix_number=${s.matrix_number}`).then(r=>r.json()).then(d=>{
        ...
        }else document.getElementById('vh').innerHTML='<p style="color:var(--slate-400);text-align:center;padding:20px;font-size:13px">No visit history.</p>';
    }).catch(()=>{document.getElementById('vh').innerHTML='<p style="color:var(--red-600)">Error loading history.</p>';});
}
</script>
</body></html>