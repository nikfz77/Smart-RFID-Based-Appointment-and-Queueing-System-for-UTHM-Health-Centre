...
<!DOCTYPE html><html lang="en"><head>
    ...
    <title>Patient List — PKU UTHM</title>
    ...
    <style>
        *,*::before,*::after{margin:0;padding:0;box-sizing:border-box}
        ...
        .logo-mark{width:32px;height:32px;background:var(--blue-500);border-radius:8px;display:flex;align-items:center;justify-content:center;flex-shrink:0;color:#fff}
        ...
        .stat-icon{width:40px;height:40px;border-radius:9px;background:var(--blue-50);display:flex;align-items:center;justify-content:center;flex-shrink:0;color:var(--blue-600)}
        .stat-icon.green{background:var(--green-100);color:var(--green-600)}.stat-icon.amber{background:var(--amber-100);color:var(--amber-600)}
...
<aside class="sidebar">
    ...
    <nav class="sidebar-nav">
        ...
        <a href="patient_list.php"           class="nav-link active"><span class="ico"><span class="material-symbols-outlined">personal_injury</span></span> Patients</a>
    ...
    <div class="sidebar-footer"><div class="user-card">
        ...
        <a href="logout.php" style="font-size:20px;color:rgba(255,255,255,.3);text-decoration:none;margin-left:auto;" title="Logout"><span class="material-symbols-outlined">logout</span></a>
    </div></div>
</aside>
...
<header class="topbar">
    <div>
        ...
        <div class="breadcrumb"><a href="staff_dashboard.php">Dashboard</a><span class="sep">›</span><span>Patients</span></div>
    </div>
...
<div class="page"><div class="page-body">
    ...
    <div class="stats-row" style="grid-template-columns:repeat(5,1fr)">
        <div class="stat-card">
            <div class="stat-icon"><span class="material-symbols-outlined">personal_injury</span></div>
            <div><div class="stat-val"><?php echo $total_patients; ?></div><div class="stat-lbl">Total Patients</div></div>
        </div>
        <div class="stat-card">
            <div class="stat-icon" style="background:#dbeafe;color:#1d4ed8"><span class="material-symbols-outlined">school</span></div>
            <div><div class="stat-val" style="color:#1d4ed8"><?php echo $total_student_type; ?></div><div class="stat-lbl">Students</div></div>
        </div>
        <div class="stat-card">
            <div class="stat-icon green"><span class="material-symbols-outlined">badge</span></div>
            <div><div class="stat-val" style="color:var(--green-600)"><?php echo $total_staff_type; ?></div><div class="stat-lbl">Staff</div></div>
        </div>
        <div class="stat-card">
            <div class="stat-icon" style="background:#dcfce7;color:#15803d"><span class="material-symbols-outlined">person_add</span></div>
            <div><div class="stat-val" style="color:#15803d"><?php echo $new_this_month; ?></div><div class="stat-lbl">New This Month</div></div>
        </div>
        <div class="stat-card">
            <div class="stat-icon amber"><span class="material-symbols-outlined">search</span></div>
    ...
    <div class="filter-bar">
        <form method="GET"><div class="filter-row">
            ...
            <div style="display:flex;gap:8px;align-items:flex-end">
                <button type="submit" class="btn btn-primary"><span class="material-symbols-outlined">search</span> Search</button>
                <a href="patient_list.php" class="btn btn-outline"><span class="material-symbols-outlined">close</span> Clear</a>
            </div>
        </div></form>
    </div>
    ...
    <div class="table-card">
        ...
        <div class="empty-state">
            <div style="font-size:32px;margin-bottom:10px;opacity:.4"><span class="material-symbols-outlined" style="font-size:32px">inbox</span></div>
            <p>No patients found</p>
        </div>
        <?php else: ?>
        <div style="overflow-x:auto"><table class="data-table">
            <thead><tr><th>#</th><th>Matrix</th><th>Name</th><th>Type</th><th>Faculty</th><th>Year</th><th>Email</th><th>Visits</th><th>Last Visit</th><th>Action</th></tr></thead>
            <tbody>
            ...
            <tr>
                ...
                <td><?php
                    $email = $s['email']??'';
                    if(str_contains($email,'@student.uthm.edu.my')){
                        echo '<span style="padding:2px 8px;border-radius:5px;font-size:11px;font-weight:700;background:#dbeafe;color:#1d4ed8">Student</span>';
                    } elseif(str_contains($email,'@uthm.edu.my')){
                        echo '<span style="padding:2px 8px;border-radius:5px;font-size:11px;font-weight:700;background:#dcfce7;color:#15803d">Staff</span>';
                    } else {
                        echo '<span style="padding:2px 8px;border-radius:5px;font-size:11px;font-weight:700;background:#f1f5f9;color:#64748b">Other</span>';
                    }
                ?></td>
                ...
                <td><button class="btn btn-sm btn-outline" onclick='showHistory(<?php echo json_encode($s); ?>)'><span class="material-symbols-outlined" style="font-size:14px">history</span> History</button></td>
            </tr>
            <?php endforeach; ?>
            </tbody>
        </table></div>
...
<div class="modal-overlay" id="modal" onclick="if(event.target===this)this.classList.remove('active')">
    <div class="modal">
        <div class="modal-hd">
            <h3><span class="material-symbols-outlined" style="color:var(--blue-600);margin-right:6px;vertical-align:middle">history</span>Patient History</h3>
            <button class="modal-close" onclick="document.getElementById('modal').classList.remove('active')"><span class="material-symbols-outlined">close</span></button>
        </div>
